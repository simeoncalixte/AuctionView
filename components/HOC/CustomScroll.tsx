...
const ThumbTrack = styled.div<{calculatedHeight: number, position: number}>`
    position: relative;
...
const withScrollBar = (WrappedComponent: React.FunctionComponent) => {
    ...
    return (props) => {
        ...
        const MouseBodyClicked = ( e ) => {
            ...
            if(Numbers.isBetween(clickYPosition, pageDownRegion.start, pageDownRegion.end)){
                ScrollBodyReference.current.scrollTop += clientHeight;
                throttle = throttleLimit;

            }else if (Numbers.isBetween(clickYPosition, pageUpRegion.start, pageUpRegion.end)){
                ScrollBodyReference.current.scrollTop -= clientHeight;
                throttle = throttleLimit;
            }
        }
        ...
        const onMouseMove = isDraggingThumbTrack ? scrollBodyTrackDragPosition: null;

        React.useEffect(()=>{
            if (ScrollBodyReference.current) {
                const {scrollHeight, scrollTop, clientHeight} = ScrollBodyReference.current
