...
const updateModelState = (
  ...
  vendorModels.forEach((Model) => {
    ...
    if (!objectToManage[Model.vendor_id]) {
      ...
      objectToManage[Model.vendor_id][Model._id] = {
        colors,
        modelDetail,
      };
    }
...
const ModelFilters = (props) => {
  ...
  const fetchModels = () => {
    ...
    if (queuesExist) {
      ...
      modelRequest(reducedSelection).then(updateLookUp);
    }
  };
  ...
  const updateContext = (vendor, model) => {
    ...
    if (!copiedFilters[MODELS]) {
      ...
      if (!copiedFilters[MODELS][vendor]) {
        copiedFilters[MODELS][vendor] = {
          [model]: modelsLookUp[vendor][model],
        };
      } else {
    ...
    const modelList = Object.keys(modelsLookUp).map((vendor) => {
      ...
      const i = models.map((modelName) => {
        ...
        const isChecked =
          ...
          selectedFilters["Models"][vendor] &&
          selectedFilters["Models"][vendor][model._id];
