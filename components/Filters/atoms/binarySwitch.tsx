...
const SwitchContainer = styled.div`
  ...
  position: relative;
  background: #2827273d;
  background: linear-gradient(60deg, #00000094, #fff6),
    linear-gradient(180deg, #0000009e, #ffffff6b);
`;
...
const Switch = styled.div<{ binary: boolean }>`
  ...
  ${(props) => (props.binary ? `left: 0px;` : `right: 0px;`)}
`;
...
const BinarySwitch = (props: IBinarySwitch) => {
  let timeOut = null;
  const [binary, setBinaryValue] = React.useState(
